...
// ------------------------------------------------------------
Function plot_raw_panel2(color_mode)
    Variable color_mode

    // print "raw panel created"    // DEBUG — remove before publication
    String current_folder = GetDataFolder(1)
    String trace_folder   = current_folder + "Trace:"
    String stim_folder    = current_folder + "Stim:"
    ...
    String trace_last_name = StringFromList(nTrace-1, trace_list)
    String stim_win        = panel_name + "#Stim"
    String trace_win       = panel_name + "#Traces"

    // Cursors on Stim — x_range from stim_last
    place_cursors(stim_win,  stim_last,  0.3, 0.7)

    // Cursors on Traces — x_range from trace_last
    Variable x_range_trace = rightx(trace_last) - leftx(trace_last)
...
Function plot_raw_panel(color_mode)
    ...
    // Color scale — only for Cividis mode
